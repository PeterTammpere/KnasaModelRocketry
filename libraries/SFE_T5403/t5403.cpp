...
// Base library type
{
	_interface = MODE_I2C; //set I2C used for communication
}
...
uint8_t T5403::acquisitionCommandTemperature(void)
{
...
void T5403::rawDataRetrieveTemperature(void)
...
int16_t T5403::convertAndGetTemperature(temperature_units units)
// Return a temperature reading.
{
...
{
	sensorWait( acquisitionCommandTemperature() );
	rawDataRetrieveTemperature();
	return convertAndGetTemperature(units);
}


	
uint8_t T5403::acquisitionCommandPressure(uint8_t commanded_precision)
{
	...
	switch(commanded_precision)
	{
		case MODE_LOW:
		{	
			delayTimeMs = 5; //  Wait 5ms for conversion to complete
			break;
		}
		case MODE_STANDARD:
		{
			delayTimeMs = 11; //  Wait 11 ms for conversion to complete
			break;
		}
		case MODE_HIGH:
		{
			delayTimeMs = 19; //  Wait 19 ms for conversion to complete
			break;
		}
		case MODE_ULTRA:
		{
			delayTimeMs = 67; //  Wait 67 ms for conversion to complete
			break;
		}
	}
	return delayTimeMs;
...
void T5403::rawDataRetrievePressure(void)
{
	//  Receive raw pressure value from device.
	getData(T5403_DATA_REG, (int16_t*)&pressure_raw);	
}

int32_t T5403::convertAndGetPressure( void )
{
	// Return a pressure value based on raw temperature and raw pressure readings
...
{
	// First acquire raw temperature from sensor
	sensorWait( acquisitionCommandTemperature() );
	rawDataRetrieveTemperature();

	// Then acquire raw pressure from sensor
	sensorWait( acquisitionCommandPressure(precision) );
	rawDataRetrievePressure( );
	
	// Use raw temperature and raw pressure to get real pressure
	return convertAndGetPressure();
}
